terraform {
  ...
  backend "s3" {
    key     = "terraform-new-snark-propagation-time.tfstate"
    encrypt = true
    region  = "us-west-2"
...
locals {
  testnet_name = "new-snark-propagation-time"
  ...
  ledger_config_location = "/home/izzy/repos/coda/dlog-only/genesis_ledgers/phase_three/qa_net_config.json" 
}

variable "coda_image" {
  type = string
  default = "codaprotocol/coda-daemon:0.0.12-beta-feature-dlog-only-13683da"
}
