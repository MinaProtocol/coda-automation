locals {
  ...
  coda_repo      = "stable"
  coda_version   = "0.0.11-beta1+314227-release-0.0.12-beta-c570287-PV13ea18ef" # Note: '*' gets latest when specifying version
...
terraform {
  ...
  backend "s3" {
    key     = "test-net/terraform-ballmer-echo.tfstate"
    encrypt = true
    region  = "us-west-2"
